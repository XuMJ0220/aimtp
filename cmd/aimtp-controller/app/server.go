...
// NewAIMTPCommand 创建一个 *cobra.Command 对象，用于启动应用程序.
func NewAIMTPServerCommand() *cobra.Command {
	// 创建默认的应用命令行选项
	...
	cmd := &cobra.Command{
		...
		// 设置命令运行时的参数检查，不需要指定命令行参数。例如：./aimtp param1 param2
		Args: cobra.NoArgs,
	}
	...
	cmd.PersistentFlags().StringVarP(&configFile, "config", "c", filePath(), "Path to the aimtp configuration file.")
	...
	// 添加 Informer 子命令
	cmd.AddCommand(NewInformerCommand(opts))
