...
import (
	...
	handler "aimtp/internal/aimtp_server/handler/grpc"
)
...
func (c *ServerConfig) NewGRPCServerOr() (server.Server, error) {
	...
	grpcsrv, err := server.NewGRPCServer(
		...
		func(s grpc.ServiceRegistrar) {
			// 注册服务器
			apiv1.RegisterAIMTP_SERVERServer(s,handler.NewHandler())
		},
		c.cfg.TLSOptions,
	)
	if err != nil {
	...
	httpsrv, err := server.NewGRPCGatewayServer(
		...
		func(mux *runtime.ServeMux, conn *grpc.ClientConn) error {
			// 注册 Handler
			return apiv1.RegisterAIMTP_SERVERHandler(context.Background(), mux, conn)
		},
		c.cfg.TLSOptions,
	)
	if err != nil {
