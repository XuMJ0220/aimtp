...
import (
	...
	handler "aimtp/internal/apiserver/handler/grpc"
...
func (c *ServerConfig) NewGRPCServerOr() (server.Server, error) {
	...
	grpcsrv, err := server.NewGRPCServer(
		...
		func(s grpc.ServiceRegistrar) {
			apiv1.RegisterAIMTPServer(s, handler.NewHandler(c.biz))
		},
		c.cfg.TLSOptions,
	)
	if err != nil {
	...
	httpsrv, err := server.NewGRPCGatewayServer(
		...
		func(mux *runtime.ServeMux, conn *grpc.ClientConn) error {
			return apiv1.RegisterAIMTPHandler(context.Background(), mux, conn)
		},
		c.cfg.TLSOptions,
	)
	if err != nil {
...
func NewAuthnWhiteListMatcher() selector.Matcher {
	whitelist := map[string]struct{}{
		apiv1.AIMTP_Healthz_FullMethodName:    {},
		apiv1.AIMTP_CreateUser_FullMethodName: {},
		apiv1.AIMTP_Login_FullMethodName:      {},
	}
